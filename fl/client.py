...
class Client:
    ...
    #     noisy_grad_list = []
    #
    #     # Define DP noise parameters based on data perturbation radius
    #     dp_noise_scale = 1/512 # Adjust based on data normalization
    #
    #     def add_dp_noise(param, scale, noise_type='laplace'):
    #         """Function to add Gaussian or Laplace noise to gradients."""
    #         if noise_type == 'gaussian':
    #             noise = torch.normal(mean=0, std=scale, size=param.grad.shape, device=param.grad.device)
    #         elif noise_type == 'laplace':
    #             noise = torch.distributions.Laplace(loc=0, scale=scale).sample(param.grad.shape).to(param.grad.device)
    #         else:
    #             raise ValueError("Unsupported noise type. Use 'gaussian' or 'laplace'.")
    ...
    #         loss = self.CE_criterion(pred, y)
    #
    #         # Calculate and save raw gradients
    #         client_grad = torch.autograd.grad(loss, self.model.parameters(), retain_graph=True)
    #         original_grad = list((g.detach().clone() for g in client_grad))
    #         grad_list.append(original_grad)
    #         loss.backward()
    #
    #         if clipDP > 0:
    #             # Gradient clipping before adding noise
    ...
    #                 add_dp_noise(param, dp_noise_scale, noise_type='laplace')
    #
    #         # Optional: Clip gradients after adding noise
    #         if clipDP > 0:
    #             torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=clipDP, norm_type=2)
    #
    ...
    #         self.tb_writer.add_scalar(f'C{self.id}/train_acc', acc, comm_R)
    #
    #         # Save noisy gradients (optional, for analysis)
    #         noisy_grad = [param.grad.detach().clone() for param in self.model.parameters() if param.grad is not None]
    #         noisy_grad_list.append(noisy_grad)
    #
    #         self.frozen_net(True)
    #
    #     return loss_val_list, acc_val_list, grad_list, noisy_grad_list

    def perform_em_train(self, x, y, comm_R, l, u, warming_up=False, nfl_lba=10.,
                          clipDP=-1, u_loss_type='direct', privacy_measure='nfl', optimized_target='val',
                          element_wise_rand=True, dp_upratio=2):
        ...
        # 隐私参数
        num_classes = 4 # 类别数量c(假设为10类，根据需要调整)
        q = 1.0  # 采样率
        noise_scale = 0
        C = 1.0 if clipDP <= 0 else clipDP

        batch_indices = range(x.size(0))
        ...
        for i in range(self.local_batch_iter):
            ...
            for idx in batch_indices:
                # 获取单个样本
                single_x = x[idx:idx + 1]
                single_y = y[idx:idx + 1]

                # 前向传播
                pred = self.model(single_x)
                loss = self.CE_criterion(pred, single_y)

                # 计算该样本的梯度
                sample_grad = torch.autograd.grad(loss, self.model.parameters(), create_graph=False)

                # 转换为张量列表以便操作
                sample_grad = [g.clone() for g in sample_grad]

                # 随机选择一个类别r
                r = torch.randint(0, num_classes, (1,)).item()

                # 为所有类别j≠r的梯度分量设置为0
                for param_idx, grad in enumerate(sample_grad):
                    # 这里简化处理，假设输出层相关参数的第一维是类别维度
                    if len(grad.shape) >= 1:
                        # 创建所有元素为0的掩码
                        mask = torch.zeros_like(grad)

                        # 如果是输出层且形状匹配，只保留r类的梯度
                        if grad.shape[0] == num_classes:
                            mask[r] = 1.0
                        else:
                            # 对于非输出层，保留所有梯度
                        ...
                        # 应用掩码
                        sample_grad[param_idx] = grad * mask

                # 计算梯度的L2范数
                flat_grads = torch.cat([g.flatten() for g in sample_grad])
                grad_norm = torch.norm(flat_grads)

                # 进行梯度缩放
                scaling_factor = torch.minimum(torch.tensor(1.0), C / (grad_norm + 1e-10))
                scaled_grads = [g * scaling_factor for g in sample_grad]

                all_sample_grads.append(scaled_grads)

            # 聚合所有样本的梯度
            aggregated_grads = []
            for param_idx in range(len(all_sample_grads[0])):
                # 对每个参数聚合所有样本的梯度
                param_grads = [grads[param_idx] for grads in all_sample_grads]
                agg_grad = sum(param_grads) / len(param_grads)
                aggregated_grads.append(agg_grad)

            # 保存原始梯度用于记录
            original_grad = [g.clone() for g in aggregated_grads]
            grad_list.append(original_grad)
            ...
            for grad in aggregated_grads:
                # 计算敏感度因子
                sensitivity = C / (q * len(batch_indices))

                noise = torch.normal(0, noise_scale * 2 * C * sensitivity, grad.shape, device=grad.device)
            ...
            noisy_grad_list.append([g.clone() for g in noisy_grads])
            ...
            self.tb_writer.add_scalar(f'C{self.id}/train_loss', loss_val, comm_R)
            self.tb_writer.add_scalar(f'C{self.id}/train_acc', acc, comm_R)
            self.tb_writer.add_scalar(f'C{self.id}/ldpm_batch_size', len(batch_indices), comm_R)
