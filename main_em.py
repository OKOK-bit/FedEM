...
def fed_init(args, tb_writer, ds_name, shuffle):
    ...
    # data_path = '/tmp/pycharm_project_196/fednfl-master/data/fmnist'
    # data_path = '/tmp/pycharm_project_196/fednfl-master/data/mnist'
    # data_path = '/home/chen/tmp/pycharm_project_580/fednfl-master/data/organamnist'
    data_path = '/tmp/pycharm_project_344/fednfl-master/data/octmnist'
...
def dlg_inv_grad(args, ground_truth, labels, original_model, input_grad, tb_writer, cid, bid, train_epoch_round,
                 input_type='raw'):
    ...
    if input_type == 'updates':
        rec_machine = inversefed.FedAvgReconstructor(original_model, (dm, ds), args.local_batch_iter, args.lr,
                                                     invgrad_config,
                                                     use_updates=True, num_images=args.batch_size)
    else:  # raw, equiv
        rec_machine = inversefed.GradientReconstructor(original_model, (dm, ds), invgrad_config,
                                                       num_images=args.batch_size)
    output, stats = rec_machine.reconstruct(input_grad, labels, img_shape=img_shape, tb_writer=tb_writer, cid=cid,
                                            bid=bid)
    ...
    test_ssim = float(
        np.mean([calculate_ssim(torch.unsqueeze(x, 0).cpu(), torch.unsqueeze(x_rec, 0).cpu()).cpu().detach().item()
...
def fed_train():
    ...
    #     lr=args.lr_def, weight_decay=args.weight_decay_def, momentum=args.momentum_def)
    # # train_loader = utils.get_indexed_loader(
    # #     args.dataset, batch_size=args.batch_size2, root=args.data_dir, train=True)
    # # test_loader = utils.get_indexed_loader(
    # #     args.dataset, batch_size=args.batch_size2, root=args.data_dir, train=False)
    ...
    #     lower_bound=args.pgd_lowbound,  # 扰动下界
    #     ascending = False,
    # )
    #
    # if not args.cpu:
    ...
    for train_epoch_round in range(args.global_epoch):
        ...
        for batch_idx, clients_batch_data in enumerate(zip(*train_loader_list)):
            ...
            for client_idx, c_batch_data in enumerate(clients_batch_data):
                ...
                # ===0.2 初始化扰动
                ''' init model / optim / dataloader / loss func '''
                model = clients[0].get_copied_model()
                optim = utils.get_optim(
                    args.optim, model.parameters(),
                    lr=args.lr_def, weight_decay=args.weight_decay_def, momentum=args.momentum_def)
                # train_loader = utils.get_indexed_loader(
                ...
                #对抗训练生成扰动过程
                for step in range(start_step, args.train_steps_def):

                    def_x = defender.perturb(model, criterion, x, y)

                    # def_noise[:def_x.shape[0]] = (def_x - x).cpu().numpy()

                    # # print(x.min(), x.max())
                    # # perturbation = def_x - x
                    # # print(perturbation.min(), perturbation.max())  # 输出扰动范围
                    #
                    # # # 可视化原始图像和添加扰动后的图像
                    # # visualize_images(x[0], def_x[0])  # 仅选择第一个图像进行展示

                    # if args.cpu:
                    #     def_x = x + torch.tensor(def_noise[:def_x.shape[0]], device=x.device)
                    # else:
                    #     def_x = x + torch.tensor(def_noise[:def_x.shape[0]], device=x.device).cuda()
                ...
                # # 可视化原始图像和添加扰动后的图像
                # visualize_images(x, def_x)  # 仅选择第一张图像进行展示
                ...
                if train_epoch_round <= args.nfl.warm_up_rounds - 1:
                    loss_list, acc_list, grad_list, noisy_grad_list = clients[client_idx]. \
                        perform_nfl_train(def_x, y, comm_R, l, u, warming_up=True,
                    ...
                    if 'em' in args.nfl.apply_distortion:
                        loss_list, acc_list, grad_list, noisy_grad_list = clients[client_idx]. \
                            perform_em_train(def_x, y, comm_R, l, u, warming_up=False,
                                              u_loss_type=args.nfl.u_loss_type,
                                              nfl_lba=args.nfl.lba, clipDP=args.nfl.clipDP,
                                              privacy_measure=args.nfl.privacy, optimized_target=args.nfl.opt_target,
                        ...
                        loss_list, acc_list, grad_list, noisy_grad_list = clients[client_idx]. \
                            perform_nfl_train(def_x, y, comm_R, l, u, warming_up=False,
                                              u_loss_type=args.nfl.u_loss_type,
                                              nfl_lba=args.nfl.lba, clipDP=args.nfl.clipDP,
                                              privacy_measure=args.nfl.privacy, optimized_target=args.nfl.opt_target,
                                              element_wise_rand=args.nfl.element_wise_rand,
                ...
                if args.nfl.apply_dlg and batch_idx == 0 and client_idx == 0 and train_epoch_round in args.nfl.dlg_attack_epochs:
                    ...
                    # torch.save(dlg_save_dict,
                    #            os.path.join(save_path, f'C{client_idx}_E{train_epoch_round}_B{batch_idx}.pkl'))
        ...
        for i in range(args.n_clients):
            ...
            logger.info(
                "client:%2d, test acc:%2.6f, best epoch:%2d, loss:%2.6f" % (i, test_accs[i], best_rounds[i], loss_avg))
